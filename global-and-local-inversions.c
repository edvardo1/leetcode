...
int CountInverse( no_t *no, int valor ) {
    if ( no == NULL ) {
        return 0;
	}
	...
	if ( no->valor > valor ) {
		if ( no->dir == NULL ) {
    	    return 1 + CountInverse( no->esq, valor );
		} else {
    	    return 1 + no->dir->tamanho + CountInverse( no->esq, valor );
    	}
	}

    return CountInverse( no->dir, valor );
}
...
bool isIdealPermutation(int *nums, int numsSize) {
	...
	for ( int i = 0; i < numsSize; i++ ) {
		global += CountInverse( no, nums[i] );
		no = Inserir_r( no, nums[i] );
	}
...
int main( void ) {
	...
	int esize = 30000;
	int *t11 = malloc(esize * sizeof(int));
	for (int i = 0; i < esize; i++) {
		t11[i] = i;
	}
	srand(0xdeadbeef);
	for (int i = 0; i < esize; i++) {
		t11[rand() % esize] = t11[rand() % esize];
	}
	...
	passou += RodarTeste( 11, t11, esize, false );
	free(t11);
